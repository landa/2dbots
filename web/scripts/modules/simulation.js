import { T, isTraversable } from './constants.js';
import * as utils from './utils.js';

/**
 * Simulation: Defines a 2dBots Simulation Environment
 */
class Simulation {

  /**
   * Instantiates a new simulation environment.
   *
   * Currently a Simulation is coupled with its visualization using p5.js.
   *
   * @param {T[][]} grid a grid of map cells
   * @param {DOMElement} simulationContainer where to draw the simulation
   * @param {DOMElement} outputContainer where to write any errors or logs
   */
  constructor(grid, simulationContainer, outputContainer) {
    ...
    // Grid
    this.grid = grid;
    this.occupancyGrid = utils.fillGrid(this.grid, null);
  ...
  addRobot(robot, x, y, d) {
    if (!utils.inBounds(this.grid, x, y)) {
      return false;
    }
  ...
  canMoveForwardRobot(robot_idx) {
    ...
    let boundCheck = utils.inBounds(this.grid, x, y);
    if (!boundCheck) {
      return false;
    }
  ...
  drawGrid() {
    ...
    for (let jj = 0; jj < this.grid.length; ++jj) {
      for (let ii = 0; ii < this.grid[0].length; ++ii) {
        ...
        } else if (value == T.E) { // empty
          this.sketch.fill(100, 100, 100);
        } else if (value == T.C) { // candy
          this.sketch.fill(100, 100, 100);
        } else if (value == T.D) { // dirt
          this.sketch.fill(100, 100, 100);
        } else if (value == T.S) { // start
          this.sketch.fill(100, 100, 100);
        } else if (value == T.T) { // target
          this.sketch.fill(100, 100, 100);
        }
        ...
        if (value == T.C) {
          ...
          this.sketch.circle(x + this.TILE_WIDTH_PIXELS / 2, y + this.TILE_WIDTH_PIXELS / 2, this.TILE_WIDTH_PIXELS / 2);
        } else if (value == T.S || value == T.T) {
          this.sketch.noFill();
          if (value == T.S) {
            this.sketch.stroke(200, 200, 200);
          } else {
  ...
  mouseWheel(event) {
    this.simulation.drawBackground();
